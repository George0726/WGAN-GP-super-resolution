...
def train():
    ...
    for epoch in range(0, n_epoch + 1):
        ## update learning rate(cannot change,  must be zhengshu)
...
def evaluate(imid):
    ...
    valid_lr_imgs = tl.vis.rid_hr_img_list = sorted(tl.files.load_file_list(path=config.VALID.hr_img_path, regx='.*.png', printable=False))
    valid_lr_img_list = sorted(tl.files.load_file_list(path=config.VALID.lr_img_path, regx='.*.png', printable=False))
    #valid_hr_imgs = ead_images(valid_lr_img_list, path=config.VALID.lr_img_path, n_threads=32)
    # for im in valid_lr_imgs:
    #     print(im.shape)
    ...
    #valid_hr_img.dtype = 'float32'
    #valid_hr_img.dtype = 'float32'
    #valid_hr_img1= np.array(Image.open(save_dir + '/valid_hr.png'),'f')
    #with tf.Session() as sess:
    #    valid_hr_img = tf.image.convert_image_dtype(valid_hr_img, dtype=tf.float32)
    #test_mse = tl.cost.mean_squared_error(out[0], valid_hr_img, is_mean=True)
